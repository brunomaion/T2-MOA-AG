...
typedef struct St_DefGene{
   ...
   float Min;  //Menor valor real assumível no gene
	float Max;  //Maior valor real assumível no gene
}TDefGene;
...
public:
	...
	float Min;       //Valor mínimo para o gene
	float Max;       //Valor máximo para o gene
	double Valor;  //Valor associado ao gene

	Gene(){}
	Gene(const int &Tam, const float &Mn, const float &Mx); //Gera os alelos binários
	void Eval();
